...
// don't permit direct requests of this file
if ( ! defined('ABSPATH') ) {
...
class xml_simple {
    ...
    function xml_simple($encoding = 'UTF-8') {
        ...
        // xml_set_object($this->parser, &$this); // Austin removed pass-by-reference so as not to throw PHP 5 errors
    ...
    function char_data($parser, $data) {
		...
		( 
			empty($this->tree) || 
			empty($this->tree[0]) || 
			empty($this->tree[0]['content']) || 
			! is_string($this->tree[0]["content"]) 
		) && 
	! preg_match("/\\S/", $data) ) 
		return;
	if ( empty( $this->tree[0]['content'] ) ) {
		$this->tree[0]['content'] = '';
	}
